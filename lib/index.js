/**
 * dsh-runner-watcher — DSH (DeepSeek Harness) plugin entry.
 *
 * Hosts self-hosted GitHub Actions runner observability inside DSH:
 *
 *   tools       — runner_watcher_* tools the agent can call directly
 *   webServer   — a dashboard served at /dsh-runner-watcher (optional service)
 *
...
export function apply(ctx, config = {}) {
  ...
  const settings = {
    ...
    liveIntervalSec: Number(config.liveIntervalSec ?? DEFAULT_SETTINGS.liveIntervalSec ?? 0),
    autoCollectOnIdle: config.autoCollectOnIdle ?? DEFAULT_SETTINGS.autoCollectOnIdle ?? false,
    autoCollectCooldownSec: Number(
      config.autoCollectCooldownSec ?? DEFAULT_SETTINGS.autoCollectCooldownSec ?? 600,
    ),
  };
  const session = () => openSession({ dataDir, settings });

  ctx.logger?.info?.(`[runner-watcher] data directory: ${dataDir}`);

  ctx.tools.register({
    name: 'runner_watcher_status',
    description:
    ...
    output: stringOutput('Runner Watcher status'),
    execute: async (args) => {
      const s = await session();
      const status = await s.liveStatus();
      if (!status.runners.length) {
        return `No runners registered yet (data dir: ${dataDir}).\nImport one with runner_watcher_add, or call runner_watcher_discover to find them automatically.`;
      }
  ...
  ctx.tools.register({
    ...
    output: stringOutput('Runner Watcher registry'),
    execute: async () => {
      const s = await session();
      ...
      if (pending.length) {
        ...
        lines.push('', 'Register them with runner_watcher_adopt (all, or an id list).');
      }
      return lines.join('\n');
    },
  });
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        wslDistro: {
          type: 'string',
          description:
            'Run commands inside this WSL distro on the remote Windows host (kind=ssh), e.g. "Ubuntu"',
        },
    ...
    output: stringOutput('Runner Watcher add'),
    execute: async (args) => {
      const s = await session();
      ...
      if (args.wslDistro) transport.wslDistro = args.wslDistro;
      ...
      return [
        ...
        '',
        'Run runner_watcher_collect to ingest its job history.',
      ].join('\n');
    },
  });

  ctx.tools.register({
    name: 'runner_watcher_remove',
    ...
    output: stringOutput('Runner Watcher remove'),
    execute: async (args) => {
      const s = await session();
  ...
  ctx.tools.register({
    ...
    output: stringOutput('Runner Watcher discover'),
    execute: async (args) => {
      const s = await session();
      ...
      for (const conflict of a.conflicts) {
        lines.push(`WARNING duplicate agent ${conflict.key}: ${conflict.ids.join(', ')} — remove one with runner_watcher_remove`);
      }
      for (const error of result.errors) lines.push(`error   ${error.transport}: ${error.message}`);
      if (a.pending.length && !result.adopted) {
        lines.push('', 'Call runner_watcher_adopt to register the pending candidates.');
      }
      return lines.join('\n');
    },
  });
  ...
  ctx.tools.register({
    name: 'runner_watcher_adopt',
    description: 'Register pending runners found by runner_watcher_discover. Pass ids/labels to choose, or omit to adopt all.',
    parameters: {
      type: 'object',
      properties: {
    ...
    output: stringOutput('Runner Watcher adopt'),
    execute: async (args) => {
      const s = await session();
      const adopted = await s.adoptPending(args?.selectors ?? []);
      if (!adopted.length) return 'Nothing pending matches; run runner_watcher_discover first.';
      return ['adopted:', ...adopted.map((r) => `- ${r.id}  ${r.label}  ${r.transport.path}`), '', 'Run runner_watcher_collect to ingest job history.'].join('\n');
    },
  });

  ctx.tools.register({
    name: 'runner_watcher_collect',
    description:
    ...
    output: stringOutput('Runner Watcher collect'),
    execute: async (args) => {
      const s = await session();
  ...
  ctx.tools.register({
    ...
    output: stringOutput('Runner Watcher analysis'),
    execute: async (args) => {
      const s = await session();
      const stats = await s.analyze({ hours: args?.hours });
      if (!stats.completed) {
        return `No completed jobs in the last ${stats.windowHours}h. Run runner_watcher_collect first.`;
      }
  ...
  ctx.tools.register({
    ...
    output: stringOutput('Runner Watcher dashboard'),
    execute: async (args) => {
      const s = await session();
  ...
  ctx.inject(['webServer'], (wctx) => {
    // Near-real-time cache: a background timer refreshes it every
    // liveIntervalSec, and `api/live` only ever reads it — a dashboard
    // request never triggers a sweep, whatever the endpoint.
    let liveCache = { ts: null, runners: [], errors: [] };
    let liveRefreshing = false;
    // Auto-collect bookkeeping: a finished job is visible as busy -> not-busy,
    // so the live tick knows when the job store is worth refreshing. The timer
    // never blocks the tick: the collection runs after the cache is published.
    let collectRunning = false;
    let autoCollectCount = 0;
    let lastAutoCollectAt = null;
    const runAutoCollect = async () => {
      collectRunning = true;
      autoCollectCount += 1;
      lastAutoCollectAt = Date.now();
      try {
        const s = await session();
        // Discovery is skipped: the registry is already known here, and this
        // pass exists to ingest the job that just finished.
        await s.collect({ autoDiscover: false, writeDashboardFile: true });
      } catch {
        /* a failed pass keeps the previous store; the next tick retries */
      } finally {
    ...
    const refreshLive = async () => {
      ...
      let next = null;
      try {
        next = await collectLiveStatus(await session());
      } catch {
        /* keep the previous cache; the client does the same */
      } finally {
        liveRefreshing = false;
      }
      if (!next) return;
      const previous = liveCache;
      liveCache = next;
      if (!settings.autoCollectOnIdle) return;
      const completions = detectBusyToIdle(previous.runners, next.runners);
      const allowed = shouldAutoCollect({
        now: Date.now(),
        lastAt: lastAutoCollectAt,
        cooldownMs: settings.autoCollectCooldownSec * 1000,
        running: collectRunning,
        completions,
      });
      if (allowed) runAutoCollect();
    };
    let liveTimer = null;
    if (settings.liveIntervalSec > 0) {
      liveTimer = setInterval(refreshLive, Math.max(3, settings.liveIntervalSec) * 1000);
      liveTimer.unref?.();
    }
    const handler = async (req, res) => {
      try {
        ...
        if (route.startsWith('api/')) {
          ...
          if (route === 'api/live') {
            body = {
              ...liveCache,
              autoCollect: {
                enabled: settings.autoCollectOnIdle === true,
                running: collectRunning,
                count: autoCollectCount,
                lastAt: lastAutoCollectAt ? new Date(lastAutoCollectAt).toISOString() : null,
              },
            };
          } else if (route === 'api/status') body = await s.liveStatus();
        ...
        const file = await s.dashboard({ hours: settings.windowHours, refreshSeconds: settings.refreshSeconds, liveIntervalSec: settings.liveIntervalSec });
        const html = await fs.readFile(file, 'utf8');
        res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
        res.end(html);
      } catch (error) {
        res.writeHead(500, { 'content-type': 'text/plain; charset=utf-8' });
        res.end(`runner-watcher dashboard error: ${String(error?.message ?? error)}`);
      }
    };
    wctx.effect(
      () => wctx.webServer.register({ kind: 'prefix', path: DASHBOARD_PATH, handler }),
      'runner-watcher: dashboard route',
    );
    wctx.effect(
      () => (liveTimer ? () => clearInterval(liveTimer) : undefined),
      'runner-watcher: live status timer',
    );
    ctx.logger?.info?.(`[runner-watcher] dashboard at ${DASHBOARD_PATH}`);
  });
}
