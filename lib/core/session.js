 ...
 * @module dsh-runner-watcher/core/session
 */
...
export async function openSession({ dataDir, settings = {} } = {}) {
  ...
  session.dashboard = async ({ hours, outFile, refreshSeconds, liveIntervalSec } = {}) => {
    const windowHours = hours ?? session.settings.windowHours;
    const jobs = filterWindow(await loadJobs(dataDir), windowHours);
    ...
    return writeDashboard(
      {
        ...
        liveIntervalSec: liveIntervalSec ?? session.settings.liveIntervalSec ?? 0,
        dataDir,
      },
      { outFile },
    );
