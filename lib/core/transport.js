 ...
 * @module dsh-runner-watcher/core/transport
 */
...
export function normalizeTransportSpec(spec) {
  ...
  if (kind === 'ssh') {
    ...
    return {
      ...
      wslDistro: input.wslDistro ? String(input.wslDistro) : undefined,
      path: String(input.path || ''),
    };
  }
...
export function describeTransport(spec) {
  ...
  const wsl = s.wslDistro ? `(wsl:${s.wslDistro})` : '';
  return `ssh:${s.user ? `${s.user}@` : ''}${s.host}${wsl}${s.path ? `:${s.path}` : ''}`;
}

/**
 * Wrap a POSIX script so it executes inside a WSL distro on the remote Windows
 * host. The script travels base64-encoded: its text freely contains single
 * quotes, `$` expansions and redirects that PowerShell and cmd.exe would
 * otherwise mangle, while a base64 token is inert in every Windows shell.
 */
export function wrapWslScript(script, distro) {
  const encoded = Buffer.from(script, 'utf8').toString('base64');
  return `wsl.exe -d ${distro} -- bash -lc "echo ${encoded} | base64 -d | bash"`;
}
...
function createSshTransport(spec) {
  ...
  const args = (script) => {
    ...
    base.push(target, spec.wslDistro ? wrapWslScript(script, spec.wslDistro) : script);
    return ['ssh', base];
  };
